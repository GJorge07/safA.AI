...
test("contrato fixo completo fica pronto para o backend", () => {
  const extraction = extracaoContratoSchema.parse({
    ...
    clausulaOriginal: "Honorários de R$ 2.500 em 20/09/2026.", confianca: 0.99, avisos: [],
  });
...
test("contrato de êxito sem valor não vira zero", () => {
  const extraction = extracaoContratoSchema.parse({
    ...
    parcelas: [], clausulaOriginal: "20% do valor líquido recebido.", confianca: 0.9, avisos: [],
  });
...
test("contrato misto preserva parte fixa e êxito na extração rica", () => {
  const extraction = extracaoContratoSchema.parse({
    ...
    clausulaOriginal: "R$ 1.500 e 15% sobre o proveito econômico bruto.", confianca: 0.98, avisos: [],
  });
